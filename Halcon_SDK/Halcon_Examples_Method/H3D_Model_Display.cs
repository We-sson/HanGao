...
namespace Halcon_SDK_DLL.Halcon_Examples_Method
{
    ...
    public class H3D_Model_Display : IDisposable
    {
        ...
        private void Calibration_3D_SizeChanged(object sender, SizeChangedEventArgs e)
        {
            ...
            {
                H3D_Display_Message_delegate?.Invoke("窗口尺寸修改更新失败! 原因:" + _He.Message);
        ...
        private void Calibration_3D_Results_MouseWheel(object sender, MouseWheelEventArgs e)
        {
            ...
            {
                H3D_Display_Message_delegate?.Invoke("三维缩放计算失败! 原因:" + _he.Message);
        ...
        private void Event_Int(int hv_Width, int hv_Height)
        {
            ...
            catch (Exception _he)
            {
                ...
                //报错输出
                H3D_Display_Message_delegate?.Invoke("窗口尺寸更新失败! 原因:" + _he.Message);
        ...
        private  void Display_Ini()
        {
            ...
            try
            {
                ...
                {
                    throw new Exception("No OpenGL support included.");
                ...
                hv_CameraIndex = hv_Scene3D.AddScene3dCamera(hv_CamParam);
                //hv_AllInstances = hv_Scene3D.AddScene3dInstance(hv_ObjectModel3D.ToArray(), hv_PoseIn);
                Set_Scene3D_Param(hv_Scene3D, Scene3D_Param);
                //Set_Scene3D_Instance_Param(hv_Scene3D, Scene3D_Instance);
            ...
            {
                H3D_Display_Message_delegate?.Invoke("可视化窗口初始化失败! 原因:" + _he.Message);
        ...
        private void DispScene3d()
        {
            ...
            Task.Run(() =>
            {
                ...
                {
                    H3D_Display_Message_delegate?.Invoke("三维可视化渲染失败! 原因:" + _he.Message);
                ...
                finally
                {
                    ...
                    //hv_WindowHandle.Clone();
                    //hv_WindowHandleBuffer.Clone();
        ...
        private void color_string_to_rgb(HTuple hv_Color, out HTuple hv_RGB)
        {
            ...
            catch (Exception )
            {
                ...
                throw new Exception("Wrong value of control parameter Color (must be a valid color string)");
            }
        ...
        private HPose determine_optimum_pose_distance(HTuple hv_ObjectModel3DID, HTuple hv_CamParam, HTuple hv_ImageCoverage, HTuple hv_PoseIn)
        {
            ...
            HTuple hv_Row, HTuple hv_Column, HTuple hv_TextColor, HTuple hv_ButtonColor)
        {
            ...
            catch (Exception )
            {
                ...
                throw new Exception("Wrong value of control parameter ButtonColor (must be a valid color string)");
            }
            hv_Fac.Dispose();
            hv_Fac = 0.4;
            hv_RGBL.Dispose();
        ...
        private HTuple get_object_models_center()
        {
            ...
            catch (Exception _he)
            {
                ...
                H3D_Display_Message_delegate?.Invoke("计算模型中心失败! 原因:" + _he.Message);
            ...
            try
            {
                ...
                for (int hv_Index = 0; hv_Index < hv_NumModels; hv_Index++)
                {
                    ...
                    try
                    {
                        ...
                        HOperatorSet.GetObjectModel3dParams(hv_ObjectModel3D.TupleSelect(hv_Index), "diameter_axis_aligned_bounding_box", out hv_CurrDiameter);
                ...
                for (int hv_Index = 0; hv_Index < hv_NumModels; hv_Index++)
                {
                    ...
                    HOperatorSet.AffineTransPoint3d(hv_HomMat3D, hv_Center.TupleSelect(0), hv_Center.TupleSelect(1), hv_Center.TupleSelect(2),
                        out hv_TBCenterCamX, out hv_TBCenterCamY, out hv_TBCenterCamZ);


                    hv_TBCenter[0] = (hv_TBCenter.TupleSelect(0)) + (hv_TBCenterCamX * (hv_Weight.TupleSelect(hv_Index)));

                    hv_TBCenter[1] = (hv_TBCenter.TupleSelect(1)) + (hv_TBCenterCamY * (hv_Weight.TupleSelect(hv_Index)));

                    hv_TBCenter[2] = (hv_TBCenter.TupleSelect(2)) + (hv_TBCenterCamZ * (hv_Weight.TupleSelect(hv_Index)));
            ...
            out HTuple hv_CameraParamOut)
        {
            ...
            {
                throw new Exception("Different number of values in ParamName and ParamValue");
            }
            ...
            for (hv_Index = 0; (int)hv_Index <= (int)((new HTuple(hv_ParamName.TupleLength())) - 1); hv_Index = (int)hv_Index + 1)
            {
                ...
                {
                    throw new Exception("Wrong ParamName " + hv_ParamNameInd);
                }
                ...
                if ((int)(new HTuple(hv_ParamNameInd.TupleEqual("focus"))) != 0)
                {
                    ...
                    {
                        throw new Exception(new HTuple("Focus for telecentric lenses is always 0, and hence, cannot be changed."));
                    }
                    if ((int)((new HTuple(hv_IsTelecentric.TupleNot())).TupleAnd(new HTuple(((hv_ParamValue.TupleSelect(
                        hv_Index))).TupleEqual(0.0)))) != 0)
                    {
                        throw new Exception("Focus for non-telecentric lenses must not be 0.");
                    }
                }
            }
