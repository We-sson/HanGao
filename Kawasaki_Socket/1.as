...
.END
.PROGRAM autostart1.pc() #0; #0; #0; #0; #0; #0; #0; #0; 通信主程序
  port = 60000
  ip[1] = 192
  ip[2] = 168
  ip[3] = 44
...
.END
.PROGRAM autostart5.pc() #0; #0; #0; #0; #0; #0; #0; #0; 通信主程序
  port = 60000
  ip[1] = 192
  ip[2] = 168
  ip[3] = 44
...
;CALL close_socket; 断开通信
  END
.END
.PROGRAM close_err() #0; #20; #180; #21; #109; #748; #0; #0
;sock_error = 0
;sock_error = ERROR
  TYPE "关闭错误！"
  RETURNE
.END
.PROGRAM close_socket() #0; #9; #0; #3; #24; #17;断开通信程序
;*****************************************
;* FUNCTION: 套接字关闭程序              *
;* WorkType: TCP/IP通讯                  *
...
; IFPWPRINT 1,2,10,4,10 = "通讯关闭"
  ...
  END
.END
.PROGRAM com_test() #0; #0; #0; #0; #0; #0; #0; #4; 根据指示数据进行各处理
  CASE text_id OF
   VALUE 0: ; 无指示
    GOTO break_exit
   VALUE 1: ; 指示处理 1
...
break_exit:
.END
.PROGRAM main() #0; #0; #0; #0; #0; #0; #0; #0; 通信主程序
  port = 60000
  ip[1] = 192
  ip[2] = 168
  ip[3] = 44
...
exit_end:
.END
.PROGRAM mes_close_socke() #0; #0; #0; #0; #0; #0; #0; #38402;断开通信
  ...
  END
.END
.PROGRAM mes_int() #0; #0; #0; #0; #0; #0; #0; #0; #0; #37520; Mes初始化
;数据初始化
  work_a_state = FALSE;
  work_b_state = FALSE;
  work_c_state = FALSE;
  work_d_state = FALSE;
.END
.PROGRAM mes_main() #0; #0; #0; #0; #0; #0; #0; #0; 通信主程序
; max_length = 255
;tout_rec = 5
; eret = 0
  sock_id = 0
...
;END
.END
.PROGRAM mes_open_socket() #0; #0; #0; #0; #0; #0; #0; #101; #0; #0;开始通信
  .er_count = 0
  .ret = 0
  .port = 60000
  .ip[1] = 192
...
;sock_id= TCP_CONNECT (sock_id,.port,.ip[1]);超时1秒
  ...
  END
.END
.PROGRAM open_socket() #0; #3; #0; #4; #24; #11;通讯连接程序
;*****************************************
;* FUNCTION: 通讯连接程序                *
;* WorkType: TCP/IP通讯                  *
;* Copyright[c]2022 by KRCT              *
...
.END
.PROGRAM recv() #0; #0; #0; #0; #0; #0; #0; #257168; 通信 接收数据
  .num = 0
  ...
  END
.END
.PROGRAM send(.ret,.$data) #0; #0; #0; #0; #0; #0; #0; #4; 通信 发送数据
  $send_buf[1] = .$data
  buf_n = 1
  .ret = 1
send_rt:
  ...
  END
.END
.PROGRAM sock_err() #0; #14; #180; #20; #109; #748; #0; #0
  sock_error = 0
  sock_error = ERROR
  ONE close_err
  TCP_CLOSE sock_close,sock_id
